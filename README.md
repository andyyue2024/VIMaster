...
| **LLM 专家** | 6 位分析专家分析 | ✅ | 基本面、情绪、估值、技术、风险管理、投资组合管理等专业分析 |
...
基于大语言模型，专注于特定分析领域的专业分析：

| # | 专家 Agent | 分析领域 | 提示词文件 |
|:-:|-----------|----------|------------|
| 1 | **Fundamentals Agent** | 盈利能力、增长情况、财务健康、估值比率 | `master11.txt` |
| 2 | **Sentiment Agent** | 内部交易信号、新闻情绪分析 | `master12.txt` |
| 3 | **Valuation Expert Agent** | DCF 估值、所有者收益估值 | `master13.txt` |
| 4 | **Technical Agent** | 趋势跟踪、均值回归、动量、波动率、统计套利 | `master14.txt` |
| 5 | **Risk Manager Agent** | 投资组合价值、头寸限制、风险控制 | `master15.txt` |
| 6 | **Portfolio Manager Agent** | 综合决策、订单生成、多空交易 | `master16.txt` |
...
命令:
  ...
  3. experts <股票代码>     - 使用分析专家 LLM 分析
  4. portfolio <股票1> <股票2> ... - 分析股票组合
  5. buy <股票1> <股票2> ... - 获取买入推荐
  6. help              - 显示帮助
  7. exit              - 退出程序
============================================================
...
|------|------|
| 首页 | 功能入口、特色展示、快速开始按钮 |
| 股票分析 | 9 大 Agent 综合分析，热门股票快捷按钮 |
| 🎓 大师分析 | 7 位投资大师 LLM 分析，支持选择特定大师 |
| 👔 专家分析 | 6 位分析专家 LLM 分析，多维度专业分析 |
| 投资组合 | 批量输入，预设组合，汇总统计 |
| 📄 报告生成 | PDF/Excel 报告，组合报告，报告历史 |
| 历史记录 | 搜索过滤、历史回顾 |
| ⚙️ 设置 | **LLM 配置（模型选择、API 密钥）**、数据源、Agent 配置 |
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v2.0 | 2026-01-27 | **LLM Agent 系统**：7 位投资大师 Agent + 6 位分析专家 Agent，支持多种 LLM 提供商；**Web UI 增强**：8 个页面，集成大师/专家分析、社区、报告等 |
